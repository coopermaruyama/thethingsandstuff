...
function __destruct()
{
    ...
    /*
     * CUSTOM [EnvyWeb] Resize Images to square
     */
    public function cropToSquare($location = 'center')
    {
        if ($this->_imageSrcWidth == $this->_imageSrcHeight)
        {
            return;
        }
        ...
        if ($this->_imageSrcWidth > $this->_imageSrcHeight)
        {
            if ($location == 'center')
            {
                $posX = ($this->_imageSrcWidth - $this->_imageSrcHeight) / 2;
                $posX = ceil($posX);
                $posY = 0;
            }
            ...
            {
                $posX = ($this->_imageSrcWidth - $this->_imageSrcHeight);
                $posY = 0;
            }

            $newWidth = $this->_imageSrcHeight;
            $newHeight = $this->_imageSrcHeight;
        }
        else if ($this->_imageSrcHeight > $this->_imageSrcWidth)
        {
            if ($location == 'center')
            {
                $posX = 0;
                $posY = ($this->_imageSrcHeight - $this->_imageSrcWidth) / 2;
                $posY = ceil($posY);
            }
            else if ($location == 'left')
            {
            ...
            {
                $posX = 0;
                $posY = ($this->_imageSrcHeight - $this->_imageSrcWidth);
            }

            $newWidth = $this->_imageSrcWidth;
            $newHeight = $this->_imageSrcWidth;
        }
        ...
        imagecopy($canvas, $this->_imageHandler, 0, 0, $posX, $posY, $this->_imageSrcWidth, $this->_imageSrcHeight);
