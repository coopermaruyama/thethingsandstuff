...
class Trego_Ajaxfilter_Block_Catalog_Layer_Filter_Price extends Mage_Catalog_Block_Layer_Filter_Price 
{
	...
	*/
	public function setProductCollection(){
		// BEGIN edited to allow condition & price tier filters [EnvyWeb]
		if (isset($_GET['condition']) && !isset($_GET['pt'])) { //only condition is set
			$conditionArray = preg_split( "/,/", urldecode($_GET['condition']) );
			if($this->_currentCategory){
				$this->_productCollection = $this->_currentCategory
				->getProductCollection()
				->addAttributeToSelect('*')
				->addAttributeToFilter('condition', array('in' => $conditionArray)) //set condition hack [EnvyWeb]
				->setOrder('price', 'ASC');
			}else{
		...
		else if (!isset($_GET['condition']) && isset($_GET['pt'])) { //only price tier is set
			$priceTierArray = preg_split("/,/", urldecode($_GET['pt']) );
			if($this->_currentCategory){
				$this->_productCollection = $this->_currentCategory
				->getProductCollection()
				->addAttributeToSelect('*')
				->addAttributeToFilter('price_tier', array('in' => $priceTierArray)) //set condition hack [EnvyWeb]
				->setOrder('price', 'ASC');
			}else{
				$this->_productCollection = Mage::getSingleton('catalogsearch/layer')->getProductCollection()	
				->addAttributeToSelect('*')
		...
		else if (isset($_GET['condition']) && isset($_GET['pt'])) { //both are set
			$conditionArray = preg_split( "/,/", urldecode($_GET['condition']) );
			$priceTierArray = preg_split("/,/", urldecode($_GET['pt']) );
		...
		// END edited to allow condition & price tier filters [EnvyWeb]			
	}
